...
def runstrat(args=None):
    ...
    YahooData = bt.feeds.YahooFinanceData   # 默认从 Yahoo Finance 获取数据
    if args.offline:
    ...
    d1 = cerebro.resampledata(data0,    # 重放数据
                              timeframe=getattr(bt.TimeFrame, args.timeframe))
    d1.plotinfo.plotmaster = data0
    d1.plotinfo.sameaxis = True # 重放数据与原数据在同一坐标轴上

    if args.pandascal:
        cerebro.addcalendar(args.pandascal) # 添加交易日历
    elif args.owncal:
...
def parse_args(pargs=None):
    ...
    parser.add_argument('--data0', default='YHOO',  # 数据源
                        required=False, help='Data to read in')

    parser.add_argument('--offline', required=False, action='store_true',   # 从本地文件读取数据
                        help='Read from disk with same name as ticker')

    # Defaults for dates
    parser.add_argument('--fromdate', required=False, default='2016-01-01', # 开始日期
                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')

    parser.add_argument('--todate', required=False, default='2016-12-31',   # 结束日期
                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')
    ...
    pgroup = parser.add_mutually_exclusive_group(required=False)    # 创建互斥组
    pgroup.add_argument('--pandascal', required=False, action='store',    # 交易日历
                        default='', help='Name of trading calendar to use')

    pgroup.add_argument('--owncal', required=False, action='store_true',    # 自定义交易日历
                        help='Apply custom NYSE 2016 calendar')

    parser.add_argument('--timeframe', required=False, action='store',  # 时间帧
